...
export class Markdown {
    ...
    // Fix внешних ссылок VK вида:
    // [#alias|example.com/index...|https://example.com/index...]
    text = text.replace(/\[#alias\|([^|\]]+)\|([^\]]+)]/g, (match, title, link) => {
        return `[${title}](${link})`;
    });

    // Fix обычных VK-ссылок вида:
    // [club123|Название]
    // [id123|Имя]
    text = text.replace(/\[([^|\[\]]+)\|([^\]]+)]/g, (match, link, title) => {
        return `[${title}](${!link.startsWith(LINK_PREFIX) ? LINK_PREFIX : ''}${link})`;
    });
